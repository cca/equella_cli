...
const postOrPutCb = (url, status, cb) => {
    const fn = cb || console.log
    if (url) return fn(url)
    ...
    if (options.method === 'put' || options.method === 'post') {
        ...
        return fetch(makeURL(options), {
            ...
            if (resp.status === 200 || resp.status === 204) {
                ...
                if (cb && cb !== defaultCb) return cb(resp.json())
            } else {
                console.log(`HTTP ${resp.status} — Error deleting ${resp.url}`)
                return resp.json()
            }
        }).then(err => {
            if (err) handle(null, err)
        }).catch(err => handle(err))
    } else {
        return fetch(makeURL(options), {
            headers: headers,
            method: 'get' // TODO cca/equella_cli#25 support HEAD & OPTIONS
        }).then(resp => {
